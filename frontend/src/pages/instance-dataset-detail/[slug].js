...
export default function Page({params}) {
    ...
    const [ getInstanceDataSetInfo ] = useGetInstanceDataSetInfoMutation()
    ...
    useEffect(() => {
        const onGetInstanceDataSetInfo = async () => {
          if (user && user?.email) {
            ...
            try {
              const res = await getInstanceDataSetInfo(formData)
        ...
        <Box className={CGroups.settings_layout}>
            <CardBox>
                ...
                <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} >
                    ...
                    <Grid item xs={12} sm={6}>
                        <Typography variant="button" gutterBottom sx={{ display: 'block' }}>
                            Instance Wise Class Balance Score
                        </Typography>
