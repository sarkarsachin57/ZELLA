...
class SemanticSegmentationAugmentor:
    def __init__(self, dataset_root, new_dataset_path, augmentations, target_classes=None, n_augments=None, aug_configs=None):
        ...
        self.class_list = json.loads(open(os.path.join(self.dataset_root, "metadata.npy")).read())["classes"]
    ...
    def augment_images(self):
        ...
        if isinstance(self.target_classes, list):
            ...
            for img_file_name in img_list:
                ann_file_name = ".".join(img_file_name.split(".")[:-1])+".npy"
                ann_path = os.path.join(ann_dir, ann_file_name)
                seg_map = np.load(ann_path)
                ann_classes = [self.class_list[class_id] for class_id in np.unique(seg_map).tolist()]
        ...
        if isinstance(self.target_classes, list):
            for class_name in self.target_classes:
                ...
                for image_file_name in selected_images:
                    ...
                    ann_path = os.path.join(ann_dir, ".".join(image_file_name.split(".")[:-1])+".npy")
                    ...
                    # Apply augmentation
                    augmented = augment_pipeline(image=img, mask=seg_map)
                    aug_img = augmented['image']
                    aug_seg_map = augmented['mask']
                    ...
                    aug_ann_path = os.path.join(self.new_annotations_path, 
                                                f"{'.'.join(image_file_name.split('.')[:-1])}_aug_{aug_uuid}.npy")

                   
                    np.save(aug_ann_path, aug_seg_map)
